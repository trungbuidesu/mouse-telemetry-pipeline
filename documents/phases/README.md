...
| Decision record | `documents/decisions/decisionNNN-<short-name>.md` | Quyết định kiến trúc hoặc performance cần giữ ổn định |
| Task file | `documents/phases/<phase>/taskNNN-<short-name>.md` | Nhật ký thực thi một task cụ thể |

Một plan phải trỏ về phase sở hữu nó. Nếu plan dựa trên một quyết định kiến trúc, plan phải liên kết tới decision record tương ứng trong `documents/decisions/`.
...
3. Các public contract như schema, API endpoint, Kafka topic hoặc storage layout đã được document.
