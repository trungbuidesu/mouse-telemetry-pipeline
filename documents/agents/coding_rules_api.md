...
* Python 3.12.
* uv-managed CPython pinned bằng `.python-version`.
* FastAPI.
* Pydantic v2.
* `pydantic-settings` cho cấu hình typed.
* `orjson` cho JSON response path khi phù hợp và có profiling.
...
`request path` là đường đi đồng bộ của một HTTP request từ lúc API nhận payload đến lúc trả response. Đường này phải ngắn, tránh analytics nặng và tránh sync disk I/O.
